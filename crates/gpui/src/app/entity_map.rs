...
impl EntityMap {
    ...
    pub(super) fn end_lease_erased(&mut self, id: EntityId, mut lease: LeaseInner) {
        self.end_lease_inner(id, lease.entity.take().unwrap());
    }

    #[inline(never)]
    fn read_inner(&self, id: EntityId) -> Option<&dyn Any> {
        let mut accessed_entities = self.accessed_entities.borrow_mut();
        accessed_entities.insert(id);
        self.entities.get(id).map(Box::as_ref)
    }

    #[inline(never)]
    fn lease_inner(&mut self, id: EntityId) -> Option<Box<dyn Any>> {
        self.accessed_entities.get_mut().insert(id);
        self.entities.remove(id)
    }

    #[inline(never)]
    fn end_lease_inner(&mut self, id: EntityId, entity: Box<dyn Any>) {
